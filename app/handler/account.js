...
module.exports = async function (req, res) {
  ...
  var idExisting = 0
  await
    redisClient.existsAsync('id:' + req.params.sessionId).then(function (reply) {
      idExisting = reply
    }).catch(function (err) {
  ...
  if (idExisting) {
    var account = ''
    await
      redisClient.getAsync('id:' + req.params.sessionId).then(function (reply) {
        account = reply
      }).catch(function (err) {
        logsys.error('get usr account name error: ' + err)
      })
    await
      redisClient.hgetallAsync('usr:' + account).then(function (reply) {
        out = reply
      }).catch(function (err) {
    ...
    out['purchase'] = []
    await
      redisClient.smembersAsync('usr:' + account + ':delivery').then(async function (replies) {
    ...
    await
      redisClient.smembersAsync('usr:' + account + ':purchase').then(async function (replies) {
        for (let tx of replies) {
          await redisClient.hgetallAsync('tx:' + tx).then(function (reply) {
            if (reply) {
